...
class _SouscriptionPageState extends State<SouscriptionPage> {
  ...
  @override
  void dispose() {
    for (final c in [_marqueCtrl,_modeleCtrl,_typeCtrl,_anneeCtrl,
  ...
  // ══════════════════════════════════════════════════
  // PICKERS — vérification simple (image valide)
  // ══════════════════════════════════════════════════
  Future<XFile?> _pick() async {
    if (kIsWeb) return _picker.pickImage(source: ImageSource.gallery, imageQuality: 88);
    XFile? f;
    await showModalBottomSheet(
      ...
      builder: (_) => SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        ListTile(
          ...
          onTap: () async { Navigator.pop(context); f = await _picker.pickImage(source: ImageSource.camera, imageQuality: 88); }),
        ListTile(
          leading: const Icon(Icons.photo_library_outlined, color: Color(0xFF1A56DB)),
          title: const Text('Choisir depuis la galerie'),
          onTap: () async { Navigator.pop(context); f = await _picker.pickImage(source: ImageSource.gallery, imageQuality: 88); }),
  ...
  bool _isImageValide(String? name) {
  ...
  Future<void> _pickCarteGrise() async {
    final f = await _pick();
    if (f == null) return;
    if (!_isImageValide(f.name)) {
      _snack('❌ Format invalide. Utilisez JPG ou PNG.'); return;
    }
    final bytes = await f.readAsBytes();
    setState(() { _carteGrise = f; _carteGriseBytes = bytes; });
  ...
  Future<void> _pickPhotoVehicule() async {
    ...
    final bytes = await f.readAsBytes();
    setState(() { _photoVehicule = f; _photoVehiculeBytes = bytes; });
  ...
  Future<void> _pickPermis() async {
    ...
    final bytes = await f.readAsBytes();
    setState(() { _permis = f; _permisBytes = bytes; });
  ...
  Future<void> _soumettreVehicule() async {
    ...
    if ([_marqueCtrl,_modeleCtrl,_anneeCtrl,_immatCtrl,_chassisCtrl,_puissanceCtrl,_placesCtrl]
        .any((c) => c.text.isEmpty)) { _snack('Remplissez tous les champs obligatoires (*).'); return; }
    if (annee == null || annee < 1990 || annee > DateTime.now().year) { _snack('Année invalide.'); return; }
    ...
    try {
      final res = await _service.creerVehicule(
        ...
        numeroChassis: _chassisCtrl.text.trim().toUpperCase(), energie: _energie,
        genre: _genre, type: _typeCtrl.text.trim().isEmpty ? null : _typeCtrl.text.trim(),
      ...
      if (res['success'] == true) { _vehiculeId = res['vehicule']['id'].toString(); setState(() => _etape = 2); }
  ...
  Future<void> _calculerPrime() async {
    ...
    setState(() => _isLoading = true);
    try {
      final rp = await _service.uploadPermis(permisBytes: _permisBytes!, permisNom: _permis!.name, vehiculeId: _vehiculeId!);
      if (rp['success'] != true) { _snack(rp['message'] ?? 'Erreur upload permis.'); return; }
      final rPrime = await _service.calculerPrime(typeAssurance: _typeAssurance, vehiculeId: _vehiculeId!, dureeMois: _dureeMois);
      if (rPrime['success'] == true) { _prime = double.tryParse(rPrime['prime_montant'].toString()); setState(() => _etape = 3); }
  ...
  Future<Uint8List?> _exportSig() async {
    try {
      ...
      final p = Paint()..color = const Color(0xFF1A56DB)..strokeWidth = 3..strokeCap = StrokeCap.round..style = PaintingStyle.stroke;
  ...
  // ÉTAPE 0 : TYPE
  // ══════════════════════════════════════════════════
  Widget _etape0() => SingleChildScrollView(padding: const EdgeInsets.all(24), physics: const BouncingScrollPhysics(),
    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      const Text('Type d\'assurance', style: TextStyle(fontSize:22, fontWeight:FontWeight.w900, color:Color(0xFF1535A8))),
      const SizedBox(height:6),
      const Text('Choisissez la couverture adaptée à votre véhicule', style: TextStyle(fontSize:13, color:Color(0xFF8492A6))),
      const SizedBox(height:24),
      _typeCard('TOUS_RISQUES', Icons.security_rounded, 'Tous Risques',
          'Couverture maximale : accidents, vol, incendie, bris de glace.', const Color(0xFF1A56DB), true),
      const SizedBox(height:14),
      _typeCard('TIERS', Icons.shield_outlined, 'Responsabilité Civile',
          'Couverture obligatoire : dommages causés aux tiers uniquement.', const Color(0xFF22C55E), false),
      const SizedBox(height:28),
      const Text('Durée du contrat', style: TextStyle(fontSize:15, fontWeight:FontWeight.w700, color:Color(0xFF1535A8))),
      const SizedBox(height:12),
      Row(children: [
      ...
      child: AnimatedContainer(duration: const Duration(milliseconds:200), padding: const EdgeInsets.all(18),
        ...
        child: Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment:CrossAxisAlignment.start, children:[
            Row(children:[
              Text(title, style: TextStyle(fontWeight:FontWeight.w800, fontSize:15, color: sel?color:const Color(0xFF1A1A2E))),
  ...
  // ÉTAPE 1 : VÉHICULE
  // ══════════════════════════════════════════════════
  Widget _etape1() => SingleChildScrollView(padding: const EdgeInsets.all(24), physics: const BouncingScrollPhysics(),
    child: Column(crossAxisAlignment:CrossAxisAlignment.start, children:[
      const Text('Informations du véhicule', style: TextStyle(fontSize:22, fontWeight:FontWeight.w900, color:Color(0xFF1535A8))),
      const SizedBox(height:6),
      const Text('Remplissez les informations de votre véhicule', style: TextStyle(fontSize:13, color:Color(0xFF8492A6))),
      const SizedBox(height:20),

      // ── Carte grise (upload simple) ──────────────────
      ...
      const SizedBox(height:12),
      _field(_anneeCtrl, 'Année de fabrication *', Icons.calendar_today_outlined, type:TextInputType.number, max:4),
      const SizedBox(height:12),
  ...
  // ÉTAPE 2 : DOCUMENTS
  // ══════════════════════════════════════════════════
  Widget _etape2() => SingleChildScrollView(padding: const EdgeInsets.all(24), physics: const BouncingScrollPhysics(),
    child: Column(crossAxisAlignment:CrossAxisAlignment.start, children:[
      const Text('Documents requis', style: TextStyle(fontSize:22, fontWeight:FontWeight.w900, color:Color(0xFF1535A8))),
      const SizedBox(height:6),
      const Text('Uploadez vos documents pour valider la souscription.', style: TextStyle(fontSize:13, color:Color(0xFF8492A6))),
      const SizedBox(height:28),
      _docCard('Permis de conduire *','Obligatoire — JPG ou PNG',Icons.credit_card_rounded,_permis,_pickPermis),
      const SizedBox(height:14),
      _docCard('Carte grise','Déjà uploadée à l\'étape précédente',Icons.article_outlined,_carteGrise,_pickCarteGrise),
      const SizedBox(height:32),
  ...
  // ÉTAPE 3 : PRIME
  // ══════════════════════════════════════════════════
  Widget _etape3() => SingleChildScrollView(padding: const EdgeInsets.all(24), physics: const BouncingScrollPhysics(),
    child: Column(crossAxisAlignment:CrossAxisAlignment.start, children:[
      const Text('Votre prime d\'assurance', style: TextStyle(fontSize:22, fontWeight:FontWeight.w900, color:Color(0xFF1535A8))),
      const SizedBox(height:24),
  ...
  Widget _row(String label, String value) => Container(margin: const EdgeInsets.only(bottom:10),
  ...
  // ÉTAPE 4 : SIGNATURE
  // ══════════════════════════════════════════════════
  Widget _etape4() => SingleChildScrollView(padding: const EdgeInsets.all(24), physics: const BouncingScrollPhysics(),
    child: Column(crossAxisAlignment:CrossAxisAlignment.start, children:[
      const Text('Signature électronique', style: TextStyle(fontSize:22, fontWeight:FontWeight.w900, color:Color(0xFF1535A8))),
      const SizedBox(height:6),
      ...
      Container(
        decoration: BoxDecoration(color:Colors.white, borderRadius:BorderRadius.circular(16),
          border:Border.all(color:_strokes.isNotEmpty?const Color(0xFF1A56DB):const Color(0xFF1A56DB).withOpacity(0.4), width:2.5)),
        child: ClipRRect(borderRadius:BorderRadius.circular(14), child: SizedBox(height:220, child: Stack(children:[
          ...
          if (_strokes.isEmpty) IgnorePointer(child:Center(child:Column(mainAxisAlignment:MainAxisAlignment.center, children:[
      ...
      GestureDetector(onTap:()=>setState(()=>_sigConfirmee=!_sigConfirmee),
        child: Row(children:[
          ...
          const SizedBox(width:12),
          const Expanded(child:Text('Je confirme avoir lu et accepté les conditions générales d\'assurance AssurAncy.',
  ...
  // ÉTAPE 5 : SUCCÈS
  // ══════════════════════════════════════════════════
  Widget _etape5() => SingleChildScrollView(padding: const EdgeInsets.all(24), physics: const BouncingScrollPhysics(),
    child: Column(children:[
      const SizedBox(height:16),
      ...
      const SizedBox(height:20),
      const Text('Contrat créé avec succès !', style: TextStyle(fontSize:24, fontWeight:FontWeight.w900, color:Color(0xFF1535A8))),
      const SizedBox(height:6),
      Text(_numeroContrat??'', style: const TextStyle(fontSize:18, fontWeight:FontWeight.w700, color:Color(0xFF1A56DB))),
      const SizedBox(height:28),
      if (_qrCodeData != null) ...[
        Container(padding: const EdgeInsets.all(20),
          ...
          child: Column(children:[
            const Text('QR Code de l\'attestation', style: TextStyle(fontWeight:FontWeight.w700, fontSize:14, color:Color(0xFF1535A8))),
            const SizedBox(height:14),
      ...
      const SizedBox(height:14),
      OutlinedButton.icon(onPressed:(){}, icon: const Icon(Icons.share_rounded), label: const Text('Partager l\'attestation'),
  ...
  // HELPERS UI
  // ══════════════════════════════════════════════════
  Widget _field(TextEditingController ctrl, String hint, IconData icon, {TextInputType? type, int? max}) =>
    Container(
  ...
  Widget _drop<T>({required T value, required String label, required IconData icon,
    ...
    Container(padding: const EdgeInsets.symmetric(horizontal:16),
      ...
      child: Row(children:[
        ...
        Expanded(child: DropdownButtonHideUnderline(child: DropdownButton<T>(value:value, isExpanded:true,
          style: const TextStyle(fontSize:15, color:Color(0xFF1A1A2E)),
  ...
  Widget _btn(String label, VoidCallback onTap) => GestureDetector(onTap:onTap,
    child: Container(width:double.infinity, height:54,
      ...
      child: Center(child:Text(label, style: const TextStyle(color:Colors.white, fontSize:16, fontWeight:FontWeight.w700))),
    ),
  );
}
